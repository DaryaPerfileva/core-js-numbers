...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const scalproduct = x1 * x2 + y1 * y2;
  const veclength1 = Math.sqrt(x1 * x1 + y1 * y1);
  const veclength2 = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(scalproduct / (veclength1 * veclength2));
}
...
function getFibonacciNumber(index) {
  ...
  let LastFibonacci = 0;
  while (i <= index) {
    LastFibonacci = remember1 + remember2;
    remember2 = remember1;
    remember1 = LastFibonacci;
    i += 1;
  }
